...
import { authApi, sessionExpired, type LoginAccount } from "../services/api";
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({
    ...
    const accountToUser = (account: LoginAccount): User => ({
        ...
        organizationName: account.org_name || "Platform Central",
        orgId: account.org_id || "9eb9dedb-5433-4d45-9479-23eb965a8427",
        clinicianId: account.clinician_id || undefined,
    });
    ...
    // Listen for session-expired events dispatched by the apiFetch 401 interceptor.
    // When a refresh-token exchange also fails, the interceptor fires this event
    // so the React layer can cleanly redirect to login.
    useEffect(() => {
        const handleExpired = () => {
            authApi.logout();
            setUser(null);
