...
// Set when a 401 signs the user out, so AuthContext's demo auto-login does not
// immediately sign them straight back in.
// ponytail: this flag exists only because that auto-login does; delete both together.
const EXPIRED_KEY = "mediverify_session_expired";

export const sessionExpired = {
    get: () => sessionStorage.getItem(EXPIRED_KEY) === "1",
    clear: () => sessionStorage.removeItem(EXPIRED_KEY),
};

// An expired or revoked token invalidates every screen, not just the call that
// happened to notice, so both request paths (apiFetch and the raw-fetch SSE
// stream) route their 401 through here. A hard redirect rather than a router
// navigate: it drops all in-memory state along with the token.
...
export async function apiFetch<T>(
    ...
    // A 401 from /auth/login is a bad password, not an expired session.
...
export interface LoginAccount {
    ...
    // Set only for a clinician login: the clinicians row this user IS. Every
    // own-record route (compliance-status, reports, documents) keys off it.
...
export const policiesApi = {
    async upload(
        ...
        if (opts?.jurisdiction)
            formData.append("jurisdiction", opts.jurisdiction);
...
export const teamApi = {
    ...
    async addUsers(
        ...
        return apiFetch<StaffUserCreateResult[]>(
