...
export const ClinicianStatusPage: React.FC = () => {
    ...
    // POST /credentials/{id}/review — a clinician holds credentials:read and not
    // this, so nobody can clear their own rejected licence.
    const canOverride = hasScope(user?.role, "credentials:verify");
    ...
    // A clinician login has no roster: GET /orgs/{id}/dashboard is HR and above,
    // and its own record is the only one it may read anyway. So skip the call
    // that would 403 and go straight to its own compliance status.
    const isOwnRecordOnly = user?.role === "clinician";
    ...
    useEffect(() => {
        async function loadRoster() {
            ...
            if (isOwnRecordOnly) {
                if (!user.clinicianId) {
                    // Same condition the backend reports as "not linked to a
                    // clinician record" — say so instead of an empty roster.
                    setError(
                        "Your login is not linked to a clinician record, so there is no compliance status to show. Ask an administrator to link it.",
                    );
                } else {
                    setSelectedClinicianId(user.clinicianId);
                }
    ...
    useEffect(() => {
        ...
        async function loadCompliance() {
            try {
                ...
                setError(
                    err.message || "Failed to load clinician compliance status",
                );
            } finally {
                setLoading(false);
            }
    ...
    const clinicianName =
        statusData?.clinician?.full_name || "Clinician Record";
                ...
                <>
                    ...
                    <div className="flex items-center justify-between gap-4">
                        ...
                        <span className="text-xs text-[#6f7976] shrink-0">
                            {roster.length} total clinician
                            {roster.length === 1 ? "" : "s"} in organization
                        </span>
                    </div>
                        ...
                        <div className="overflow-x-auto">
                            {rosterLoading ? (
                                <div className="p-12 text-center text-[#57605f] flex items-center justify-center gap-2">
                                    ...
                                    <span>
                                        Loading clinician roster from backend...
                                    </span>
                                </div>
                    ...
                    <section className="bg-white rounded-xl border border-[#E2E8F0] p-6 md:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 shadow-xs">
                        ...
                        <div className="flex items-center gap-4 self-end md:self-center">
                            ...
                            {/* {canManageTeam && (
                                <ClinicianAccessControl
                                    portal_access={
                                        statusData?.clinician?.portal_access ||
                                        false
                                    }
                                    status={statusFor(selectedClinicianId)}
                                    error={errorFor(selectedClinicianId)}
                                    onGrant={() =>
                                        handleGrantAccess(selectedClinicianId)
                                    }
                                />
                            )} */}
                            {/* <button
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                            ...
                            <div className="bg-white border border-[#E2E8F0] p-4 rounded-xl shadow-xs">
                                ...
                                <p className="text-2xl font-heading font-bold text-[#16a34a] mt-1">
                                    {statusData.summary.counts_by_state
                                        ?.satisfied || 0}
                                </p>
                            </div>
                            <div className="bg-white border border-[#E2E8F0] p-4 rounded-xl shadow-xs">
                        ...
                        <div className="p-6 border-b border-[#E2E8F0] flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-[#f8f9ff]/50">
                            <div>
                                ...
                                <p className="text-xs text-[#57605f] mt-0.5">
                                    Live evaluation against policy rules and
                        ...
                        <div className="overflow-x-auto">
                            {loading ? (
                                <div className="p-12 text-center text-[#57605f] flex items-center justify-center gap-2">
                                    ...
                                    <span>
                                        Evaluating live compliance status from
                                        backend...
                                    </span>
                                </div>
                                ...
                                <div className="p-12 text-center text-[#57605f]">
                                    ...
                                    <p className="font-semibold text-[#0b1c30]">
                                        No active credential requirement gaps
                                        found
                                    </p>
                                    <p className="text-xs text-[#6f7976] mt-1">
                                        Clinician has satisfied all active
                                        policy requirements.
                                    </p>
                                </div>
                            ) : (
                        ...
                        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center p-4 z-50">
                            <div className="bg-white rounded-xl max-w-md w-full p-6 shadow-xl border border-[#CBD5E1] space-y-4">
                                ...
                                <p className="text-sm text-[#57605f]">
                                    Initiate a live real-time query against the
                                ...
                                <div className="flex justify-end gap-3 pt-3">
                                    ...
                                    <button
                                        disabled={isProcessing}
                                        onClick={() =>
                                            handleReverify(activeModal.item)
                                        }
                        ...
                        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center p-4 z-50">
                            <div className="bg-white rounded-xl max-w-md w-full p-6 shadow-xl border border-[#CBD5E1] space-y-4">
                                ...
                                <p className="text-sm text-[#57605f]">
                                    ...
                                    . This decision is immutably recorded in the
