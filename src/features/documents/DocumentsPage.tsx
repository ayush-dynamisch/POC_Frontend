...
import { documentsApi, cliniciansApi, type ReviewQueueCheck } from '../../services/api';
...
export const DocumentsPage: React.FC = () => {
  ...
  const [credentialType, setCredentialType] = useState<CredentialType>('rn_license');
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    if (!fileToUpload) {
      ...
      const dummyContent = new Blob(['%PDF-1.4 Mock Credential Document for OCR'], { type: 'application/pdf' });
      fileToUpload = new File([dummyContent], fileName || 'credential_upload.pdf', { type: 'application/pdf' });
    }
    ...
    try {
      const res = await documentsApi.upload(selectedClinicianId, fileToUpload, credentialType);
      setUploadSuccess(`Document uploaded successfully! Document ID: ${res.document_id || 'Processed'}. Status: ${res.status || 'Verified'}.`);
    ...
    <div className="p-6 md:p-8 space-y-8 max-w-7xl mx-auto">
      ...
      <div>
        ...
        <p className="text-sm text-[#57605f] mt-1">
          Upload credential certifications for multi-modal OCR extraction &amp; resolve escalated verification checks
        </p>
      </div>
      ...
      <section className="bg-white border border-[#E2E8F0] rounded-xl p-6 md:p-8 shadow-xs">
        <h2 className="font-heading font-bold text-lg text-[#0b1c30] mb-6 flex items-center gap-2">
          <span className="material-symbols-outlined text-[#0a6659] text-[22px]">upload_file</span>
        ...
        <form onSubmit={handleUpload}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="relative">
                ...
                >
                  {clinicians.length === 0 ? (
                    <option value="">Loading clinicians from backend...</option>
                  ) : (
                    clinicians.map((c) => (
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full appearance-none bg-[#f8f9ff] border border-[#CBD5E1] rounded-lg py-3 px-4 text-sm text-[#0b1c30] focus:outline-none focus:border-[#0a6659] focus:ring-2 focus:ring-[#0a6659]/20 transition-all cursor-pointer pr-10"
                >
                  <option value="rn_license">rn_license (Registered Nurse)</option>
                  <option value="md_license">md_license (Medical Doctor)</option>
                  <option value="dea_registration">dea_registration (Controlled Substance)</option>
                  <option value="bls">bls (Basic Life Support)</option>
                  <option value="acls">acls (Advanced Cardiac Life Support)</option>
                  <option value="hipaa_training">hipaa_training (Annual Security)</option>
            ...
            <div className="flex flex-col gap-2">
              ...
              <label className="w-full flex items-center justify-between bg-[#eff4ff] border border-[#CBD5E1] border-dashed rounded-lg py-3 px-4 text-sm text-[#0b1c30] hover:bg-[#dce9ff] transition-colors cursor-pointer">
                <span className="truncate mr-2 font-mono text-xs">
                  {fileName || 'Select document to upload...'}
                </span>
                <span className="material-symbols-outlined text-[#0a6659] text-[18px]">cloud_upload</span>
                <input
                  type="file"
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-t border-[#E2E8F0] pt-6">
            <p className="text-xs text-[#57605f] flex items-center gap-2 max-w-2xl">
              <span className="material-symbols-outlined text-[#0a6659] text-[18px]">info</span>
              AI pipeline will extract fields, perform tamper checks, and query the State Registry automatically.
            </p>

            <button
              type="submit"
              disabled={isUploading || !selectedClinicianId}
            ...
            >
              {isUploading ? (
                <>
                  ...
                  <span>Processing OCR &amp; Verification...</span>
                </>
              ) : (
                <>
          ...
          <div>
            <div className="flex items-center gap-3">
              <h2 className="font-heading font-bold text-lg text-[#0b1c30] flex items-center gap-2">
                <span className="material-symbols-outlined text-[#884934] text-[22px]">rule</span>
                AI Review Queue
              </h2>
            ...
            <p className="text-xs text-[#57605f] mt-1">
              Document checks scored below confidence threshold requiring Human-in-the-Loop decision
            </p>
          </div>
        </div>
        ...
        <div className="overflow-x-auto w-full">
          {isLoadingQueue ? (
            <div className="p-12 text-center text-[#57605f] flex items-center justify-center gap-2">
              ...
              <span>Loading review queue from live backend...</span>
            </div>
            ...
            <div className="p-12 text-center text-[#57605f]">
              ...
              <p className="font-semibold text-[#0b1c30]">All pending review checks are resolved!</p>
              <p className="text-xs text-[#6f7976] mt-1">No items currently below confidence threshold in the live queue.</p>
            </div>
          ) : (
            <table className="w-full text-left border-collapse min-w-[950px]">
              <thead>
                <tr className="bg-[#eff4ff]/60 border-b border-[#E2E8F0] text-xs font-semibold text-[#57605f]">
                  <th className="py-3.5 px-6">Clinician</th>
                  <th className="py-3.5 px-6">Document</th>
                  <th className="py-3.5 px-6">Check Type</th>
                  <th className="py-3.5 px-6 w-1/4">AI Finding</th>
              ...
              <tbody className="divide-y divide-[#E2E8F0] text-sm">
                {reviewQueue.map((row) => {
                  const score = row.confidence_score ?? row.score ?? 50;
                  return (
                    <tr key={row.id} className="hover:bg-[#f8f9ff] transition-colors">
                      <td className="py-4 px-6 font-medium text-[#0b1c30] whitespace-nowrap">
                        {row.clinician_name || row.clinician_id || 'Clinician'}
                      </td>
                      ...
                      <td className="py-4 px-6 text-[#57605f] whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          <span className="material-symbols-outlined text-[#6f7976] text-[18px]">
                            {(row.document_name || '').endsWith('.pdf') ? 'picture_as_pdf' : 'image'}
                          </span>
                          <span className="font-mono text-xs">{row.document_name || row.document_id || 'document'}</span>
                        </div>
                      </td>

                      <td className="py-4 px-6 text-[#0b1c30] whitespace-nowrap font-medium capitalize">
                        {(row.check_type || '').replace('_', ' ')}
                      </td>
                      ...
                      <td className="py-4 px-6 text-xs text-[#ba1a1a]">
                        <div className="flex items-start gap-1.5 font-medium">
                          ...
                          <span>{row.ai_finding || row.finding || 'Needs manual verification review'}</span>
                        </div>
                      </td>
                      ...
                      <td className="py-4 px-6 text-xs text-[#6f7976] whitespace-nowrap">
                        {row.submitted_ago || (row.created_at ? new Date(row.created_at).toLocaleDateString() : 'Just now')}
                      </td>
