...
const ALL_ROLES: Role[] = ['super_admin', 'admin', 'compliance_officer', 'hr', 'clinician'];
...
export const PolicyManagementPanel: React.FC = () => {
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (terminalEvent.step === 'complete') {
        ...
        setUploadSummary({
          policy_document_id: output.policy_document_id || res.policy_document_id,
        ...
        setUploadError(terminalEvent.message || 'Policy ingestion failed');
      }
    } catch (err: any) {
  ...
  const handleSupersede = async (policyId: string) => {
    if (!window.confirm('Supersede this published policy? Its active requirements will be retired.')) return;
    ...
    <div className="space-y-8">
      ...
      <section className="bg-white border border-[#E2E8F0] rounded-xl p-6 md:p-8 shadow-xs">
        <h2 className="font-heading font-bold text-lg text-[#0b1c30] mb-6 flex items-center gap-2">
          <span className="material-symbols-outlined text-[#0a6659] text-[22px]">gavel</span>
        ...
        <form onSubmit={handleUpload} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="flex flex-col gap-2 md:col-span-2">
              <label className="text-xs font-semibold text-[#57605f] uppercase tracking-wider">
                Policy Title <span className="text-[#ba1a1a]">*</span>
              </label>
              <input
                type="text"
                value={title}
            ...
            <div className="flex flex-col gap-2">
              <label className="text-xs font-semibold text-[#57605f] uppercase tracking-wider">File (PDF)</label>
              <label className="w-full h-11 flex items-center justify-between bg-[#eff4ff] border border-[#CBD5E1] border-dashed rounded-lg px-4 text-sm text-[#0b1c30] hover:bg-[#dce9ff] transition-colors cursor-pointer">
                <span className="truncate mr-2 font-mono text-xs">{fileName || 'Select a policy PDF...'}</span>
            ...
            <div className="flex flex-col gap-2 md:col-span-3">
              <label className="text-xs font-semibold text-[#57605f] uppercase tracking-wider">
                Visible To (optional — leave unchecked for whole organization)
              </label>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-t border-[#E2E8F0] pt-6">
            <p className="text-xs text-[#57605f] flex items-center gap-2 max-w-2xl">
              ...
              The document is chunked, embedded, and scanned for candidate requirements. It stays in draft until you review and publish it below.
            </p>
            ...
            <div className="p-4 rounded-lg bg-[#f8f9ff] border border-[#E2E8F0] text-xs space-y-1.5">
              {liveSteps.length === 0 ? (
                <div className="flex items-center gap-2 text-[#57605f]">
                  ...
                  <span>Connecting to ingestion pipeline...</span>
                </div>
              ) : (
                liveSteps.map((step) => (
                  <div key={step.step} className="flex items-center gap-2">
                    ...
                    <span className={step.status === 'done' ? 'text-[#57605f]' : 'text-[#0b1c30] font-medium'}>
                      {step.detail || STEP_LABELS[step.step] || step.step}
                    </span>
                  </div>
                ))
              )}
          ...
          {uploadSummary && (
            <div className="p-4 bg-[#dcfce7] border border-[#bbf7d0] rounded-lg text-sm text-[#166534] space-y-2">
              <div className="flex items-center gap-2 font-semibold">
                ...
                Ingestion complete — {uploadSummary.proposed_requirements ?? 0} requirement(s) extracted
              </div>
              <p className="text-xs text-[#166534]/90">
                {uploadSummary.chunks ?? 0} chunk(s), {uploadSummary.embedded ?? 0} embedded, {uploadSummary.rejected_citations ?? 0} citation(s) rejected.
                {uploadSummary.next ? ` ${uploadSummary.next}.` : ''}
              </p>
              <button
                onClick={() => setReviewPolicyId(uploadSummary.policy_document_id)}
      ...
      <section className="bg-white border border-[#E2E8F0] rounded-xl shadow-xs overflow-hidden flex flex-col">
        <div className="p-6 border-b border-[#E2E8F0] bg-[#f8f9ff]/60">
          <h2 className="font-heading font-bold text-lg text-[#0b1c30] flex items-center gap-2">
            <span className="material-symbols-outlined text-[#0a6659] text-[22px]">folder_open</span>
            Policy Corpus
          </h2>
          <p className="text-xs text-[#57605f] mt-1">Uploaded policy documents and their publication status</p>
        </div>
        ...
        <div className="overflow-x-auto w-full">
          {isLoadingPolicies ? (
            ...
            <div className="p-12 text-center text-[#ba1a1a] text-sm">{listError}</div>
          ) : policies.length === 0 ? (
            <div className="p-12 text-center text-[#57605f]">
              <span className="material-symbols-outlined text-4xl text-[#6f7976] mb-2">description</span>
              <p className="font-semibold text-[#0b1c30]">No policy documents uploaded yet.</p>
            </div>
          ) : (
              ...
              <tbody className="divide-y divide-[#E2E8F0] text-sm">
                {policies.map((p) => (
                  <tr key={p.id} className="hover:bg-[#f8f9ff] transition-colors">
                    <td className="py-4 px-6 font-medium text-[#0b1c30]">{p.title}</td>
                    <td className="py-4 px-6">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-bold uppercase border ${STATUS_BADGE[p.status]}`}>
                        {p.status}
                      </span>
                    </td>
                    <td className="py-4 px-6 text-[#57605f]">{p.jurisdiction || '—'}</td>
                    <td className="py-4 px-6 text-[#57605f] font-mono text-xs">{p.version || '—'}</td>
